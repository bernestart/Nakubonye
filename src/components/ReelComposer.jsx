...
import { X, Video, Send, Circle, Square, RotateCcw, Mic, MicOff, Camera, Upload } from "lucide-react"
...
export default function ReelComposer({ onClose, onDone }) {
  ...
  // --- Camera helpers ---
  async function startCamera(newFacing = facing) {
    try {
      // Stop old stream
  ...
  // Mount: start camera when tab = record & no recorded file
  useEffect(() => {
  ...
  function toggleMic() {
    ...
    const next = !micOn
    setMicOn(next)
    // Restart camera so audio track is right
  ...
  // ===== RENDER =====
  // If we have a file (recorded or uploaded) → show share view
  if (preview && file) {
    return (
      <div className="fixed inset-0 z-[220] bg-black flex flex-col">
        ...
        <div className="p-3 flex flex-col gap-2" style={{ paddingBottom: "max(12px, env(safe-area-inset-bottom))" }}>
          ...
          <button
            onClick={retake}
            className="h-11 rounded-full bg-white/10 text-white font-semibold text-[13.5px] inline-flex items-center justify-center gap-2"
          >
  ...
  // Tab chooser / record view
  return (
    <div className="fixed inset-0 z-[220] bg-black flex flex-col">
      <input ref={fileRef} type="file" accept="video/*" hidden onChange={pick} />

      <header className="flex items-center justify-between px-3 py-3 z-10">
        <button onClick={() => { stopCamera(); onClose() }} className="w-10 h-10 rounded-full grid place-items-center bg-black/50 text-white" aria-label="Close">
          <X size={22} />
        </button>
        <div className="flex bg-white/10 rounded-full p-1">
          <button
            onClick={() => { stopCamera(); setTab("record"); setError("") }}
            className={`h-8 px-3 rounded-full text-[12.5px] font-bold ${tab === "record" ? "bg-white text-black" : "text-white"}`}
          >
            Record
          </button>
          <button
            onClick={() => { stopCamera(); setTab("upload"); setError("") }}
            className={`h-8 px-3 rounded-full text-[12.5px] font-bold ${tab === "upload" ? "bg-white text-black" : "text-white"}`}
          >
            Upload
          </button>
        </div>
      ...
      {tab === "record" ? (
          ...
          <div className="flex-1 relative bg-black overflow-hidden">
            ...
            <div className="absolute top-4 right-4 flex flex-col gap-3">
              <button
                onClick={toggleMic}
                disabled={recording}
                className="w-11 h-11 rounded-full grid place-items-center bg-black/50 backdrop-blur-md text-white disabled:opacity-40"
            ...
            {error && (
              <div className="absolute top-20 left-4 right-4 bg-red-500/20 border border-red-500/50 rounded-xl px-3 py-2 text-red-200 text-[12.5px]">
                {error}
              </div>
            )}
          </div>
            ...
            >
              <span
                ...
                style={{
                  ...
                  background: recording ? "#EC4899" : "#fff",
                }}
              >
                {recording ? (
                  <Square size={22} fill="#fff" color="#fff" />
                ) : (
                  <Circle size={30} fill="#0B0B14" color="#0B0B14" />
                )}
              </span>
            </button>
          </div>
        ...
        <div className="flex-1 grid place-items-center p-5">
          <button
            ...
            className="w-full max-w-[420px] h-72 rounded-3xl border-2 border-dashed border-white/20 grid place-items-center"
          >
